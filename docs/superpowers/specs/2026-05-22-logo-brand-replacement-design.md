...
Replace the top navigation text brand `BaseClaw` with the provided logo image, and update the welcome page brand text from `BaseClaw` to `DeepClaw`.
...
- Update `frontend/src/components/layout/Navbar.tsx` so the left-side brand area renders the logo image instead of the blue `O` icon and `BaseClaw` text.
...
- Run a focused assertion that the navbar references `/deepclaw.png` and no longer renders `BaseClaw` in the navbar.
